...
const UserHistory = () => {
  ...
  const handleReset = () => {
    ...
    setFilters({ page: 1, limit: 10, action: '' });
  };
  ...
  const clearOwnHistory = async () => {
    if (!window.confirm('Yakin ingin menghapus semua riwayat milik Anda?')) return;
    try {
      await api.delete('/history/clear-own');
      alert('Riwayat Anda berhasil dihapus.');
  ...
  return (
    <Container className="mt-4">
      <Card>
        <Card.Body>
          {/* 🔥 Header dengan tombol hapus di kanan atas */}
          <div className="d-flex justify-content-between align-items-center mb-3">
            <Card.Title>Riwayat Aktivitas Saya</Card.Title>
            <Button 
              variant="danger" 
              size="sm"
              onClick={clearOwnHistory}
            >
              Hapus Riwayat Saya
            </Button>
          </div>
              ...
              <Form>
                <Form.Group className="mb-3">
                  ...
                  >
                    ...
                    <option value="view_report">Laporan</option>
